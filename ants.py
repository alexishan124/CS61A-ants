    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container==True and BodyguardAnt.can_contain(self.ant, insect)==True:
                    BodyguardAnt.contain_ant(self.ant, insect)
                elif self.ant.is_container==True and BodyguardAnt.can_contain(insect, self.ant)==True:
...
#         my_place = self.place
#         count = 0
#         while my_place is not hive:
#             if self.min_range <= count <= self.max_range and random_or_none(my_place.bees):
#                 return my_place.bees
#             else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bee = 0
        while place != hive:
            if self.min_range <= bee <= self.max_range and len(place.bees) != self.min_range:
